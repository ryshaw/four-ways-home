	...
	public void attachFixture(Body body, String name, FixtureDef fd, float scale) {
		...
		for (int i=0, n=rbModel.polygons.size(); i<n; i++) {
			...
			for (int ii=0, nn=vertices.length; ii<nn; ii++) {
				...
				// if it's the player main body, adjust it a lil so the feet are actually on the ground
				if (name.equals("main")) vertices[ii].sub(origin).sub(1f, 1f).add(0, 0.05f);
				else vertices[ii].sub(origin).sub(1f, 1f); // otherwise adjust it normally
			}
